...
use types::rooms::{MatrixChatSummary, MatrixGetChatNavigationResponse, MatrixRoomKind};

pub const VIRTUAL_DMS_ROOT_ID: &str = "virtual:dms";
pub const VIRTUAL_UNSPACED_ROOT_ID: &str = "virtual:unspaced";
...
pub fn orderable_root_space_ids(chats: &[MatrixChatSummary]) -> HashSet<String> {
...
pub fn build_navigation_response(
...
mod tests {
    ...
    fn derives_virtual_root_for_unspaced_room() {
        ...
        assert_eq!(
            index
                .derive_root_space_id_for_room("!orphan:example.org")
                .as_deref(),
            Some(VIRTUAL_UNSPACED_ROOT_ID)
        );
        assert_eq!(
            index
                .derive_root_space_id_for_room("!dm:example.org")
                .as_deref(),
            Some(VIRTUAL_DMS_ROOT_ID)
        );
    }

    #[test]
    fn orderable_root_space_ids_excludes_virtual_roots() {
        let chats = vec![
            chat("!space:example.org", MatrixRoomKind::Space, false, &[]),
            chat("!dm:example.org", MatrixRoomKind::Room, true, &[]),
        ];
