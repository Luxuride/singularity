...
impl AppDb {
    /// Open (or create) the encrypted application database at `path` using
    /// `secret`. Tauri-free: the binder resolves the path and secret.
    pub fn initialize(path: &std::path::Path, secret: &str) -> Result<Self, String> {
